...
export const ReviewQueueTable: React.FC<ReviewQueueTableProps> = ({
  ...
  const getReasonBadge = (item: ReviewQueueItem) => {
    if (item.evidence?.timeout) {
      return <span className="text-[10px] font-mono bg-rose-950 text-rose-300 px-2 py-0.5 rounded border border-rose-800">Pipeline Timeout</span>;
    }
    if (item.confidence < 0.5) {
      return <span className="text-[10px] font-mono bg-rose-950 text-rose-300 px-2 py-0.5 rounded border border-rose-800">Low Confidence (&lt;0.50)</span>;
    }
    if (item.confidence < 0.8) {
      return <span className="text-[10px] font-mono bg-amber-950 text-amber-300 px-2 py-0.5 rounded border border-amber-800">Medium Ambiguity</span>;
    }
    return <span className="text-[10px] font-mono bg-slate-800 text-slate-300 px-2 py-0.5 rounded">Manual Flag</span>;
  };

  return (
    <div className="rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl overflow-hidden space-y-4 p-4 sm:p-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-white placeholder-slate-500 focus:outline-none focus:border-brand-500"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          <div className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-950 hover:bg-slate-800 border border-slate-800 text-brand-300 font-mono font-semibold transition-colors"
            >
          ...
          <tbody className="divide-y divide-slate-800/60">
            {isLoading ? (
              <tr>
                <td colSpan={6} className="py-12 text-center text-slate-500">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <div className="h-6 w-6 rounded-full border-2 border-brand-500/40 border-t-brand-400 animate-spin" />
              ...
              <tr>
                <td colSpan={6} className="py-12 text-center text-slate-500">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <CheckCircle2 className="h-10 w-10 text-emerald-400/80 mb-1" />
                    <span className="text-slate-300 font-bold text-sm">Review Queue Empty!</span>
              ...
              filteredItems.map((item) => {
                ...
                return (
                  <tr
                    key={item.request_id}
                    className="hover:bg-slate-950/50 transition-colors group cursor-pointer"
                    onClick={() => onSelectItem(item)}
                  >
                    {/* Request ID */}
                    <td className="py-3 px-3 font-mono text-slate-300">
                      <span className="text-brand-300 font-semibold group-hover:text-brand-200">
                    ...
                    <td className="py-3 px-3 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-brand-600/20 hover:bg-brand-600 border border-brand-500/30 hover:border-brand-500 text-brand-300 hover:text-white font-bold text-xs inline-flex items-center gap-1.5 transition-all shadow-sm"
                      >
        ...
        <div className="flex items-center justify-between border-t border-slate-800 pt-3 text-xs text-slate-400">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 rounded-lg bg-slate-950 hover:bg-slate-800 border border-slate-800 disabled:opacity-40 transition-colors"
            >
