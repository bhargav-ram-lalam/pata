...
import { DigipinCard } from './components/DigipinCard';
import { AuditEvidenceCard } from './components/AuditEvidenceCard';
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          ...
          <div className="flex items-center gap-3 text-xs font-mono text-slate-400 shrink-0">
            <div className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl">
              ...
              <span className="text-brand-400 font-bold">5-Agent Engine</span>
            </div>
            <div className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl">
              <span className="text-slate-500">Region: </span>
              <span className="text-emerald-400 font-bold">ap-south-1 (India)</span>
            </div>
          </div>
        </div>
        ...
        {resolution && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in pt-2">
            ...
            {/* Left Column: Parsed Structure, Live Trace, DIGIPIN, Audit (7 cols) */}
            <div className="lg:col-span-7 space-y-6">
              ...
              {/* DIGIPIN Digital Postal Code */}
              <DigipinCard
                digipin={resolution.digipin}
                latitude={resolution.latitude}
                longitude={resolution.longitude}
              />

              {/* Audit Trail & DPDP Compliance */}
              <AuditEvidenceCard resolution={resolution} />
            </div>
      ...
      <footer className="border-t border-slate-900 bg-slate-950 py-6 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-400">Pata (पता)</span>
            <span>—</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <span>•</span>
            <a href="https://github.com/bhargav-ram-lalam/pata" target="_blank" rel="noopener noreferrer" className="hover:text-slate-300 transition-colors">
              GitHub Repo
            </a>
          </div>
        </div>
